...
## Current Milestone: v1.1 Embedding Second-Channel Retrieval

**Goal:** 在不破坏 lexical-first 基线和可解释性的前提下，为 ordinary retrieval 增加可选的 embedding second-channel 检索能力。

**Target features:**
- 可配置的 embedding backend 与 readiness/reporting
- ingest 后的 embedding 持久化与可选 vector sidecar/index
- lexical-first + embedding second-channel 的 recall / fusion / rerank
- 对 dual-channel 行为的 trace、诊断和回归验证
...
- [ ] 为 ordinary retrieval 增加 embedding second-channel，同时保持 lexical-first 作为默认主解释通道。
- [ ] 在 ingest/storage 层增加 embedding 持久化与可选 vector sidecar/index，但不把 semantic retrieval 变成 v1.1 的硬依赖。
- [ ] 让 search 结果和状态诊断能够解释双通道是如何参与 recall / fusion / rerank 的。
- [ ] 保持 agent-search 继续复用 ordinary retrieval 服务，而不是新建单独的 semantic bypass path。
...
v1.0 已交付一个本地优先的 Rust 记忆认知系统骨架，包含：
...
当前剩余的是非阻塞 tech debt，而不是里程碑 blocker：

- live Rig smoke 仍然是可选验证
- rumination negative-path coverage 仍可继续加深

下一里程碑 v1.1 的焦点收敛为 retrieval 扩展，而不是重新打开 cognition 或 write-back 的大范围改造：

- 把 embedding 通道做成 lexical-first 下的 second channel
- 保持 explainability、citation、status/doctor diagnostics 仍然可信
- 优先 ordinary retrieval，再让 agent-search 自然复用
...
|----------|-----------|---------|
| 项目按“普通检索 + 智能体搜索”双主线拆分 | 用户已明确要求两块能力同时存在，且理论上 recall 与 cognition 本就应分层 | ✓ Good |
| 检索底座采用 SQLite + `libsimple` + Rust 轻量关键词权重 | 更贴近当前规模和目标，避免把模型文件或语义向量作为 v1 前提 | ✓ Good |
...
| v1.1 聚焦 embedding second-channel retrieval，而不扩展到全新的接口面或治理工作流 | 先把 retrieval 双通道做对，比同时打开 MCP / HTTP / human review 更收敛 | — Pending |
| 智能体层采用 `rig`，而不是自写一层 provider glue | 后续需要模型、embedding、tool、agent 组合能力，`rig` 更适合作为 agent orchestration 层 | ✓ Good |
| 软件骨架参考 `reference/mempal` 的模块布局 | 当前仓库缺少现成代码，`mempal` 提供了合适的 Rust 模块拆分与 SQLite 搜索实践 | ✓ Good |
