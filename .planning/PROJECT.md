...
这个系统面向需要长期上下文的 AI agent 场景，尤其是编码 agent、知识工作 agent 和需要跨 session 连续性的自动化流程。项目会以 `reference/mempal` 为实现风格参考，但核心模型、领域术语和认知分层以本仓库的 0415 文档组为准，而不是简单复刻现有项目。
...
- [ ] 实现普通检索链路：支持 SQLite 本地存储、`sqlite-vec` 向量检索、`libsimple` FTS5 中文/拼音检索与可解释结果融合。
...
- `reference/mempal/`：一个完整的 Rust 参考实现，展示了单二进制、本地 SQLite、向量检索、MCP、知识图谱与模块拆分方式。它的 `core / embed / ingest / search / mcp / cowork` 分层，以及对 `sqlite-vec`、RRF 融合、taxonomy routing 的使用，对本项目的软件组织很有参考价值。

项目的实现导向应满足以下现实目标：

- 普通检索不能只是“向量搜一把”，而要兼顾中文检索体验、时间正确性、出处追踪与 recall explainability。
...
| 检索底座采用 SQLite + `sqlite-vec` + `libsimple` | 同时满足本地优先、向量检索和中文/拼音全文检索需求 | — Pending |
...
| 0415 文档组作为领域真源 | 该项目的差异化价值在认知理论，而不是通用向量库壳子 | — Pending |
