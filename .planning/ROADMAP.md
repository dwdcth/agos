...
Agent Memos 的首个里程碑按五个阶段推进：先建立 Rust + SQLite 的本地认知底座，再交付 ingest 与普通检索链路，然后把 T1/T2/T3 真值分层固化进数据模型，之后接入 working memory 与 Rig agent search，最后再补上元认知写回与双队列反刍。这样可以先把 deterministic retrieval 做对，再把 cognition 和 learning 叠上去，避免项目过早退化成“高级聊天检索”。
...
- [ ] **Phase 2: Ingest And Hybrid Retrieval** - 交付普通检索主线，打通 ingest、中文词法检索、向量检索与可解释返回
...
### Phase 2: Ingest And Hybrid Retrieval
**Goal**: 打通普通检索主线，使系统可以在不依赖 LLM 的前提下 ingest 资料并执行中文词法 + 向量混合检索。
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  ...
  2. Agent or developer can run ordinary lexical search and semantic search over the same corpus and get a fused ranked result set.
...
- [ ] 02-02: Implement `libsimple` lexical recall and `sqlite-vec` semantic recall
- [ ] 02-03: Implement result fusion, filtering, citations, and ordinary retrieval APIs
...
| 2. Ingest And Hybrid Retrieval | 0/3 | Not started | - |
